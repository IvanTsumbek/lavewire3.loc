    ...
    <form wire:submit="save" class="position-relative">
        ...
        <div wire:loading wire:target.except="save"
            style="position: absolute; width: 100%; height: 100%; 
                    background: rgba(255, 255, 255, .7); z-index: 10; text-align: center;
                    paddin-top: 20px;">
        ...
        <div class="mb-3">
            <div wire:ignore>
                <select id="country"
                    class="form-select {{-- select2 --}} @error('form.country_id') is-invalid @enderror"
        ...
        <div class="mb-3">
            <div>
                <select id="street" class="form-select @error('form.street_id') is-invalid @enderror"
                    wire:model.live='form.street_id'>
                    <option value="" selected>Select street</option>
                    @foreach ($streets as $street)
                        <option value="{{ $street->id }}" wire:key="{{ $street->id }}">{{ $street->name }}
                        </option>
                    @endforeach
                </select>
            </div>
